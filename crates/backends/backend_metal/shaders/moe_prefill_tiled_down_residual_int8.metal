...
struct MoePrefillTiledDownResidualInt8Params {
    uint N;       // hidden_size
    uint K;       // moe_intermediate_size
    uint top_k;
};

// Fused tiled prefill down projection + reduce + residual add for Int8 MoE (v2).
// Int8 per-channel: result *= scale[row].
// Uses per-(token, column) uint atomic counters instead of float CAS.

#define TILE_M 4u

[[kernel]]
void moe_prefill_tiled_down_residual_int8(
    ...
    constant MoePrefillTiledDownResidualInt8Params& p [[buffer(10)]],
    ...
    if (lane == 0) {
        ...
        for (uint mi = 0; mi < tile_count; mi++) {
            down_out[flat_idx[mi] * N + n] = routing_weight[mi] * acc[mi] * row_scale;
        }
