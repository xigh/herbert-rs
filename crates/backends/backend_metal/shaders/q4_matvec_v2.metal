...
// Q4 matvec v2 — optimised decode kernel.
//
// 3 combined optimisations vs v1:
//   1) 8 rows per threadgroup (256 threads = 8 SIMD groups) — amortises shared_x
//      load over 2× more rows, halving TG memory traffic.
//   2) Arithmetic dequant: float(nibble) - 8.0f instead of LUT[nibble].
//      Eliminates constant-memory indexed access + half→float cast.
//   3) Vectorised uint4 loads: 16 bytes (32 nibbles = 1 quantisation group)
//      per load instead of 1 uint (4 bytes = 8 nibbles). Reduces load
//      instructions 4× and matches group_size=32 perfectly (1 scale per uint4).
//
// Loop structure: each lane loads 1 uint4 (4 consecutive uints = 32 values = 1 group).
// 32 lanes × 4 uints = 128 uints per warp per iteration (stride 128).
// 2× unrolled for ILP: 256 uints per iteration.
//
// Dispatch: (ceil(N/8), 1, 1) threadgroups of 256 threads.
// TG memory: K × 2 bytes (shared_x as half).

// Process 32 Q4 values from a uint4 (1 quantisation group, 1 scale).
inline float q4_dot32(uint4 pack, float s, threadgroup const half* sx) {
...
// Process 8 Q4 values from a single uint (remainder path).
inline float q4_dot8(uint pack, float s, threadgroup const half* sx) {
    return (float((pack      ) & 0xF) - 8.0f) * s * float(sx[0])
         + (float((pack >>  4) & 0xF) - 8.0f) * s * float(sx[1])
         + (float((pack >>  8) & 0xF) - 8.0f) * s * float(sx[2])
         + (float((pack >> 12) & 0xF) - 8.0f) * s * float(sx[3])
         + (float((pack >> 16) & 0xF) - 8.0f) * s * float(sx[4])
         + (float((pack >> 20) & 0xF) - 8.0f) * s * float(sx[5])
         + (float((pack >> 24) & 0xF) - 8.0f) * s * float(sx[6])
         + (float((pack >> 28)       ) - 8.0f) * s * float(sx[7]);
}
...
void q4_matvec_v2(
    ...
    // Main loop: uint4 loads. Each lane processes 1 uint4 (32 values = 1 group).
    // 32 lanes × 4 uints = 128 uint stride per iteration.
    // 2× unrolled: process 256 uints (2048 values) per iteration.
    uint base = lane * 4;  // each lane owns 4 consecutive uints
    uint i = base;
    for (; i + 128 < num_uint; i += 256) {
        uint4 p0 = *((device const uint4*)(w_uint + i));
        uint k0 = i * 8;
        float s0 = scales[scale_row_offset + k0 / 32];
        acc += q4_dot32(p0, s0, shared_x + k0);

        uint4 p1 = *((device const uint4*)(w_uint + i + 128));
        uint k1 = (i + 128) * 8;
        float s1 = scales[scale_row_offset + k1 / 32];
        acc += q4_dot32(p1, s1, shared_x + k1);
    }

    // Handle remaining full uint4 groups
