//! Streaming shard-by-shard model loader for BF16 (no quantization).
//!
//! Delegates to the generic streaming loader in `loader_common`, passing
//! a closure that copies BF16 data directly without any quantization.
//! Supports weight caching to skip re-reading safetensors on subsequent loads.
...
const BACKEND_ID: &str = "bf16";

pub fn load_model(
    model_dir: &Path,
    opts: LoadOpts,
) -> Result<(Config, GenericModel<Bf16Ops>)> {
    ...
    if !opts.no_cache {
        if let Ok(model_hash) = weight_cache::compute_model_hash(model_dir) {
            if let Some(model) = weight_cache::load_model_cache::<Bf16Ops>(
                &config,
                model_dir,
                BACKEND_ID,
                model_hash,
    ...
    let (config, model) = loader_common::load_model_streaming_progress::<Bf16Ops>(
        model_dir,
